...
<c:choose>
                                ...
                                <div class="form-group reg_radio">
                                    ...
                                    <div class="form-check-inline">
                                        <input type="radio" class="form-check-input" name="role" value="MASTER"
                                            ${prevForm.role eq 'MASTER' ? 'checked' : ''}>Master
                                    </div>
                                    <div class="form-check-inline">
                                        <input type="radio" class="form-check-input" name="role" value="MANAGER"
                                            ${prevForm.role eq 'MANAGER' ? 'checked' : ''}>Manager
                                    </div>
                                </div>
                            </c:when>
                            <c:otherwise>
    ...
    <c:otherwise>
        <div class="col-md-12" style="text-align: center">
            <c:if test="${param.role eq 'REGISTERED_USER'}">
                <h3>You have successfully registered</h3>
                <a href="${pageContext.request.contextPath}/login">Go to the login page</a>
            </c:if>
            <c:if test="${param.role eq 'MASTER'}"><h3>The master's account has been successfully registered</h3></c:if>
            <c:if test="${param.role eq 'MANAGER'}"><h3>The manager's account has been successfully registered</h3></c:if>
        </div>
    </c:otherwise>
</c:choose>